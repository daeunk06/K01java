...
public class DBConnect {
	...
	public static void main(String[] args) {
		try {
			...
			/*
			 2-1]DriverManager클래스의 메소드를 통해 오라클에 연결을 시도
			 연결에 성공하면 주소반환함
			 */
			...
			if(con!=null) {
				...
				/*
				 3]쿼리문 작성
				 	: 쿼리문을 작성할때 주의할 점은 줄바꿈을 할때 앞뒤로 공백을 
				 	넣는게 좋다. 그렇지 않으면  syntaxError가 발생한다.
				 */
				...
				/*
				 4] 쿼리문을 Oracle로 전송하기 위한 Statement계열의
				 인터페이스 사용, 해당객체는 주로 정적쿼리를 실행할때 사용한다.
				 */
				Statement stmt = con.createStatement();
				
				/*
				 5]Statement객체를 사용하여  Oracle로 작성된 커리문을 전송하고
				 실행한다. 실행 후 결과를 반환하면 ResultSet객체를 통해 받는다.
				 */
				ResultSet rs = stmt.executeQuery(sql);
				while(rs.next()) {
					/*
					 오라클이 반한하면 ResultSet객체의 객수만큼 반복하면서
					 콘솔에 내용을 출력한다
					 getXXXX()계열의 함수는 문자, 숫자, 날짜를 사용하기위해 생성
					 인자값으로는 컬럼의 순서에 따른 인덱스를 
					 쓸 수 있고, 컬럼명을 그대로 쓸수도 있다.
					 */
