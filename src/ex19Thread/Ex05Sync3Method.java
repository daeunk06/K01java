...
/*
 동기화 메소드
 	: 여러 쓰레드에 의해 호출되는 공유메소드를 동기화 하여
 	여러 쓰레드가 동시에 접근하지 못하도록 Lock을 거는것을 말한다.
 형식] 접근지정자 synchronized반환타입 메소드명(){
 		실행문장;
 	}
 */
class MethodSyncClass
{
	synchronized void increase(int seed, int inc) {
		...
		/*
		 메소드 자체가 동기화처리되었으므로 쓰레드 실행중에 
		 sleep()을 만나더라도 다른 쓰레드는 접근 못한다.
		 */
			try {
				Thread.sleep(300);
			}
