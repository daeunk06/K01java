	...
	public static void main(String[] args) {
		/*
		 List<E> 인터페이스 계열의 컬렉션
		 	...
		 	타입매개변수를 생략하면 모든 객체를 저장할 수 있는 컬렉션이 됨
		 	즉 ArryaList<Object> 선언한것과 같다.
		 */
//		ArrayList<String> list = new ArrayList<String>();
		...
		list.add("트와이스");
		list.add(3,"워너원");
		//list.add(5,"소방차 ㅋㅋ");//에러발생(인덱스를 건너뛸 수 없다)
		...
		System.out.println("/n[확장된for문 사용]");
		for(Object obj : list) {
		...
		/*
		 1-4]덮어쓰기
		 	set(인덱스,객체) : 특정인덱스에 저장된 데이터를 덮어쓰므로
		 	기존자료는 삭제된다.
		 */
		...
		System.out.println(list.contains("블랙핑크") ? "블랙핑크있음"
				: "블랙핑크없음");
		...
		방법 1 :인덱스로 삭제하기
			remove(index) : 인덱스를 통해서 삭제를 진행한 후
				삭제가 완료되면 해당 객체를 반환.
				삭제후에는 인덱스가 자동으로 재부여된다. 삭제성공시 해당객체를
		...
		System.out.println("빅빙의 index="+index);
		list.remove(index);
		
		//방법3 : 인스턴스의 참조값을 통해 삭제
		System.out.println(list.remove("방탄소년단")? "삭제성공"
				: "삭제실패");
		list.remove("오마이걸");
