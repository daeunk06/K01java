...
/*
 HasjSet컬렉션
 	...
 	-객체 순서없이 저장
 	-객체의 중복저장을 허용하지 않음
 		딘, 기본클래스가 아닌 새롭게 정의한 클래스라면
 		hashCode(), equals()를 적절히 오버라이딩 해야한다
 	-List가 배열의 성격이라면, set은 집합.
 */
public class Ex04HashSet1 {
	public static void main(String[] args) {
		...
		int number = 100;// integer객체로 Boxing처리후 저장
		...
		/*
		 5-1] 기본 클래스형 객체 중복 저장
		 	: 기본클래스의 경우에는 별도의 오버라이딩 처리 없이 중복이 제거된다
		 	따라서 아래의 경우는  false
		 */
		...
		/*
		 5-2 개발자가 정의한 객체 중복저장
		 	...
		 	오버라이딩 하지 않으면 중복저장이 허용된다. 아래의 경우 저장이 성공하여
		 	true가 반환된다.
		 */
		Teacher teacher2 = new Teacher("김복두", 55, "체육");
		System.err.println(set.add(teacher2)?"성공":"실패");//성공
		System.out.println("[중복(teacher2)저장후 객체수]:"+set.size());//6
	
		/*
		 6]저장된 객체 출력
		 	:순서없이 저장되므로 출력도 지 멋대로 나옴
		 */
