...
/*
 함수형 인터페이스
 	: 람다식을 구현하기 위해서 먼저 인터페이스 만들고 람다식으로 구현할 추상메소드를 
 	선언한다. 이를 람다식을 만들기 위한 함수형 인터페이스라고한다.
 	※ 함수형 인터페이스에는 오직 하나의 추상메소드만 선언할 수 있다
 */

/*
 @FunctionalInterface
 	:함수형 인터페이스의 조건을 갖추었는지에 대한 검사를 컴파일러에게 요청하는 
 	역할을 하는 어노테이션
 */
interface IKosmo4{
...
public class Ex04FunctionalInterface {
	...
	/*
	 매개변수 타입이 Ikosmo4를 구현(implement)한 람다식 자체를 전달하여
