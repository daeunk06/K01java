...
public class Ex06Define2Supplier {
	...
	public static List<Integer> makeRandomNum(Supplier<Integer>s, int cnt){
		...
		//cnt크기만큼 반복하면서 난수를 생성후 list에 저장함.
		for(int i=1;i<=cnt;i++) {
			li.add(s.get());
		}
		return li;
	...
	/*
	 Supplier<T>
	 : 공급자란 의미를 가지고 있고
	 매개변수는 없으나 리턴값이 있는  get() 메소드가 정의되어있음
	 주로 실행후 호출한곳으로 데이터를 생성후 반환
	 */
	public static void main(String[] args) {
		 ...
		 */
		/*
		 위의 get()메소드를 오버라이딩하여 0~99사이의 난수를 반환하는
		 람다식 정의
		 */
		...
		int rNum = sup.get();/*
				get()메소드가 반환하는 Integer객체를 Auto UnBoxing해서
				int형변수에 저장한다
		*/
		...
		System.out.println("### foreacg문으로 출력2");
		for(Integer i : list) {
			System.out.printf("%d",i);
		}
		System.out.println();

		System.out.println("### 이레이터로 출력3");
