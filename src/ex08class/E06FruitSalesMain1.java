...
class FruitSeller{
	...
	int myMoney =0;// 판매수익
	final int APPLE_PRICE = 1000;//단가
	
	/*
	 클래스에서 멤버상수를 선언한는 경우 //멤버변수와 달리 멤버상수는 선언과 동시에 초기화 해야해
	 민액 초기화를 별도로 진행하면 에러 발생( = 1000)부분 지우면 에러떠
	 */
	
	//판매자가 사과를 판매하는 행위를 표현한 메소드 
	/*
...
class FruitBuyer{
	...
	//구매자가 판매자에게 사과를 구매하는 행위를 표현한 맴버매소드
	/*
	 1. 판매자에게 금액을 지불한다.
...
public class E06FruitSalesMain1 {
	...
	public static void main(String[] args) {
		...
		buyer.showBuyResult();
		/*
		 구매자가 판매자에게 5천원을 지불하고 사과를
		 구매한 행위를 코드로 표현함.
		 */
		buyer.buyApple(seller,5000);//구매자가 사과산다(셀러한테,오천원주고)
		
		System.out.println("구매해위가 일어난 후의 상태");
		seller.showSaleResult();
		buyer.showBuyResult();
	}
