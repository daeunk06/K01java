...
/*
 ...
 -행동의 조건
 	...
 	걸으면 에너지1감소
 	생각하면 2감소
 	단, 에너지의 최대 최소 범위는 0~10(사람은 무한히 못먹고
 	에너지가 마이너스가 되면 죽은거임)
 */
public class Human {
	...
	void walk() {
		//에너지를 1 차감한후 음수인지 확인하여 음수이면 0으로 고정
		energy--;
		if(energy<0) {
			energy =0;
	...
	void thinking () {
		...
		if(energy<0) {
			...
			System.out.println("[thinking]에너지가 1 감소하였습니다.");
	}
	//에너질를 2증가시킨후 10을 초과하면 10으로 고정시킨다.
	void eat() {
		energy += 2;
		if(energy>10) {
			energy = 10;
			System.out.println("[eat]에너지는 10이" 
					+ "최대치입니다.");
			System.out.println("0으로 고정됩니다.");
		}
		else
