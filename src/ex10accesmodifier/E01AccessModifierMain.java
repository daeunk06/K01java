...
public class E01AccessModifierMain {
	...
	public static void main(String[] args) {
		...
		/*
		 private으로 선언된 멤버는 동일한 패키지 라도 접근 불가
		 해당맴버는 동일 클래스 내에서만 접근 허용
		 ※ private 으로 선언된 멤버를 외부 클래스에서 접근할때 주로
		...
		/*
		 동일한 패키지에서 선언된 클래스 이므로 import없이 객체 생성및 접근이 가능하다.
		 */
		System.out.println("DefaultClass 객체 생성 및 접근");
		new DefaultCalss1().myFunc();// 객체 생성과 동시에 멤버메소드 호출
		...
		/*
		 다른 패키지에 선언된 클래스 이므로 객체생성을 위해 import선언을 해야한다
		...
		two.publicMethod();
		/*
		 다른 패키지에 정의된 디폴트 클래스 이므로 객체 생성 불가능
		 import도 불가능 하므로 사용할수 없는 클래스가 된다.
		 */
		
		System.out.println("DefaultClass 객체 생성및 접근");
